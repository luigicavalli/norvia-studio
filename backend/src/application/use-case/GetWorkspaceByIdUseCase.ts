...
export class GetWorkspaceByIdUseCase implements IUseCase<string, Workspace> {

    public constructor(private readonly workspaceRepository: WorkspaceRepository) {}

    public async execute(id: string): Promise<Workspace> {

        const workspace: Workspace | null = await this.workspaceRepository.findById(id);
